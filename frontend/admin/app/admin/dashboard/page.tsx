...
interface StatCard {
  ...
  icon: React.ReactNode; accent: string; link?: string; pulse?: boolean
}
...
export default function DashboardPage() {
  ...
  const stats: StatCard[] = [
    {
      label: 'Kg reciclados', value: totalKg.toFixed(0), sub: 'este mes', accent: 'bg-emerald-500',
    ...
    {
      label: 'CO₂ ahorrado', value: totalCo2.toFixed(1), sub: 'kg equivalente', accent: 'bg-sky-500',
    ...
    {
      label: 'Recolecciones', value: totalRec.toString(), sub: 'este mes', accent: 'bg-violet-500',
      icon: <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M10 2c-2.236 0-4.43.18-6.57.524C1.993 2.755 1 4.014 1 5.426v5.148c0 1.413.993 2.67 2.43 2.902 1.168.188 2.352.327 3.55.414.28.02.521.18.642.413l1.713 3.293a.75.75 0 001.33 0l1.713-3.293a.783.783 0 01.642-.413 41.102 41.102 0 003.55-.414c1.437-.231 2.43-1.49 2.43-2.902V5.426c0-1.413-.993-2.67-2.43-2.902A41.289 41.289 0 0010 2zM6.75 6a.75.75 0 000 1.5h6.5a.75.75 0 000-1.5h-6.5zm0 2.5a.75.75 0 000 1.5h3.5a.75.75 0 000-1.5h-3.5z" clipRule="evenodd" /></svg>,
    },
    {
        ...
        <>
          <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {stats.map(s => (
              <motion.div key={s.label} variants={item}>
                {s.link ? (
                  <Link href={s.link} className="block group"><StatCardInner stat={s} /></Link>
                ) : (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/admin/empresas" className="text-xs text-emerald-600 hover:text-emerald-700 font-medium">Ver todas →</Link>
            </div>

            {impacto.length === 0 ? (
              <EmptyState title="Sin datos de impacto" description="Aún no hay recolecciones aprobadas con cálculo de impacto." />
            ) : (
              <div className="space-y-4">
                {impacto.map((e, i) => (
                  <motion.div key={e.empresa_id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 + i * 0.05 }} className="flex items-center gap-4">
                    <span className={`text-xs font-bold w-5 text-center shrink-0 ${i === 0 ? 'text-amber-500' : i === 1 ? 'text-slate-400' : i === 2 ? 'text-amber-700' : 'text-slate-300'}`}>{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1.5">
                        <Link href={`/admin/empresas/${e.empresa_id}`} className="text-sm font-medium text-slate-800 hover:text-emerald-600 truncate transition-colors">
                          {e.empresa}
                        </Link>
                        <span className="text-xs text-slate-500 ml-3 shrink-0 tabular-nums">{(e.co2_ahorrado || 0).toFixed(1)} kg CO₂</span>
                      </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md transition-all duration-200 h-full">
      <div className="flex items-start justify-between mb-3">
        <div className={`w-9 h-9 rounded-xl ${stat.accent} flex items-center justify-center text-white`}>{stat.icon}</div>
        {stat.pulse && (
