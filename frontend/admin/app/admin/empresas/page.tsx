      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {loading ? '...' : `${empresas.length} empresas registradas`}
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-6 py-3.5">Empresa</th>
                <th className="text-left text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-4 py-3.5 hidden md:table-cell">Sector</th>
                <th className="text-left text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-4 py-3.5 hidden sm:table-cell">Ciudad</th>
                <th className="text-left text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-4 py-3.5 hidden lg:table-cell">Contacto</th>
                <th className="text-left text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-4 py-3.5">Estado</th>
            ...
            <tbody>
              {filtered.map((e, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-4 text-slate-500 hidden md:table-cell">{e.sector || '—'}</td>
                ...
                <tr>
                  <td colSpan={6}>
                    <EmptyState
                      ...
                      description={search ? `No se encontró ninguna empresa con "${search}"` : 'No hay empresas registradas.'}
                    />
                  </td>
                </tr>
              )}
