...
export default function EmpresaSidebar() {
  ...
  return (
    ...
    >
      ...
      {/* ── Mi empresa pill ──────────────────────────────── */}
      {profile?.nombre && (
        <div className="mx-3 mt-3 px-3 py-2.5 rounded-xl" style={{ background: 'var(--f-75)', border: '1px solid var(--f-200)' }}>
          <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'var(--f-500)' }}>
            Mi empresa
          </p>
          <p className="text-[13px] font-semibold truncate mt-0.5" style={{ color: 'var(--f-700)' }}>
            {profile.nombre}
          </p>
        </div>
      )}
      ...
      <nav className="flex-1 px-3 py-4">
        <p className="px-3 pb-3 text-[10px] font-semibold uppercase tracking-[0.1em]" style={{ color: 'var(--text-muted)' }}>
          Módulos
        </p>
            ...
            return (
              <Link key={href} href={href} className="block relative">
                ...
                <span
                  ...
                  style={{ color: active ? 'var(--f-600)' : 'var(--text-secondary)' }}
                >
                  {active && (
                    <motion.span
                      layoutId="empresa-nav-indicator"
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-full"
                      style={{ background: 'var(--f-600)' }}
                      transition={{ type: 'spring', stiffness: 500, damping: 40 }}
                    />
                  )}
                  <span style={{ color: active ? 'var(--f-600)' : 'var(--n-400)' }}>{icon}</span>
                  {label}
                </span>
              </Link>
            )
        ...
        <Link href="/empresa/perfil" className="block">
          <span
            className={`flex items-center gap-3 px-3 py-2.5 rounded-[10px] text-[13px] font-medium transition-all duration-150 ${
              path === '/empresa/perfil' ? 'bg-[var(--f-75)]' : 'hover:bg-[var(--n-75)]'
            }`}
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block truncate text-[13px]" style={{ color: 'var(--text-primary)' }}>
                {profile?.nombre ?? profile?.email ?? 'Mi cuenta'}
              </span>
      ...
      <div className="px-6 py-3">
        <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Fundares · 2026</span>
      </div>
    </aside>
  )
}
